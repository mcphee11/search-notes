...
async function createjob() {
    ...
    if (document.getElementById("wrapUp").checked) {
...
async function getjob(jobId) {
    ...
    for (const conversation of job.conversations) {
        const count = conversation.participants.filter(obj => { if (obj.participantId) { return true } return false }).length
        if (count > 1) {
                    ...
                    for (const session of participant.sessions) {
                        for (const segment of session.segments) {
                            if (segment.segmentType == "interact") {
                                ...
                                if (user != "") user = user + "<br>" + "botflow"
                                if (user == "") user = "botflow"
